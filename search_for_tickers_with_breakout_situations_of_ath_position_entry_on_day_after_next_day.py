...
def insert_sl_tp_order_price_into_df(df_with_level_atr_bpu_bsu_etc,
    ...
    timestamp_of_breakout_bar = np.nan
    open_of_breakout_bar = np.nan
    close_of_breakout_bar = np.nan
    low_of_breakout_bar = np.nan
    high_of_breakout_bar = np.nan
    volume_of_breakout_bar = np.nan

    timestamp_of_bar_before_breakout = np.nan
    open_of_bar_before_breakout = np.nan
    close_of_bar_before_breakout = np.nan
    low_of_bar_before_breakout = np.nan
    high_of_bar_before_breakout = np.nan
    volume_of_bar_before_breakout = np.nan

    timestamp_of_bar_next_day_after_breakout_bar = np.nan
    open_of_bar_next_day_after_breakout_bar = np.nan
    high_of_bar_next_day_after_breakout_bar = np.nan
    low_of_bar_next_day_after_breakout_bar = np.nan
    close_of_bar_next_day_after_breakout_bar = np.nan
    volume_of_bar_next_day_after_breakout_bar = np.nan

    open_of_bar_next_day_after_next_day_after_breakout_bar = np.nan
    close_of_bar_next_day_after_next_day_after_breakout_bar = np.nan


    try:
        timestamp_of_bar_before_breakout = \
            table_with_ohlcv_data_df_slice_numpy_array[
    ...
    try:
        timestamp_of_bar_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
                number_of_last_row_in_np_array_row_slice + 1][0]
    except:
        pass
    ...
    try:
        open_of_bar_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
                number_of_last_row_in_np_array_row_slice + 1][1]
    except:
        pass
    ...
    try:
        high_of_bar_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
                number_of_last_row_in_np_array_row_slice + 1][2]
    except:
        pass
    ...
    try:
        low_of_bar_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
                number_of_last_row_in_np_array_row_slice + 1][3]
    except:
        pass
    ...
    try:
        close_of_bar_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
                number_of_last_row_in_np_array_row_slice + 1][4]
    except:
        pass
    ...
    try:
        volume_of_bar_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
                number_of_last_row_in_np_array_row_slice + 1][5]
    except:
        pass
    ...
    try:
        open_of_bar_next_day_after_next_day_after_breakout_bar = \
            table_with_ohlcv_data_df_slice_numpy_array[
    ...
    take_profit_when_stop_loss_is_calculated_3_to_1 = (buy_order - calculated_stop_loss) * 3 + buy_order
    take_profit_when_stop_loss_is_calculated_4_to_1 = (buy_order - calculated_stop_loss) * 4 + buy_order
    # round decimals
    buy_order = round(buy_order, 20)
    calculated_stop_loss = round(calculated_stop_loss, 20)
    take_profit_when_stop_loss_is_calculated_3_to_1 = round(take_profit_when_stop_loss_is_calculated_3_to_1, 20)
    take_profit_when_stop_loss_is_calculated_4_to_1 = round(take_profit_when_stop_loss_is_calculated_4_to_1, 20)

    # stop loss is technical
    ...
    take_profit_when_stop_loss_is_technical_3_to_1 = (buy_order - technical_stop_loss) * 3 + buy_order
    take_profit_when_stop_loss_is_technical_4_to_1 = (buy_order - technical_stop_loss) * 4 + buy_order
    distance_between_technical_stop_loss_and_buy_order_in_atr = \
        distance_between_technical_stop_loss_and_buy_order / advanced_atr

    # round technical stop loss and take profit
    ...
    df_with_level_atr_bpu_bsu_etc.at[0, "take_profit_when_stop_loss_is_calculated_3_to_1"] =\
        take_profit_when_stop_loss_is_calculated_3_to_1
    df_with_level_atr_bpu_bsu_etc.at[0, "take_profit_when_stop_loss_is_calculated_4_to_1"] = \
    ...
    df_with_level_atr_bpu_bsu_etc.at[0, "take_profit_when_stop_loss_is_technical_3_to_1"] = \
        take_profit_when_stop_loss_is_technical_3_to_1
    df_with_level_atr_bpu_bsu_etc.at[0, "take_profit_when_stop_loss_is_technical_4_to_1"] = \
        take_profit_when_stop_loss_is_technical_4_to_1

    df_with_level_atr_bpu_bsu_etc.at[0, "distance_between_technical_stop_loss_and_buy_order"] = \
        distance_between_technical_stop_loss_and_buy_order

    df_with_level_atr_bpu_bsu_etc.at[0, "distance_between_technical_stop_loss_and_buy_order_in_atr"] = \
        distance_between_technical_stop_loss_and_buy_order_in_atr
    ...
    df_with_level_atr_bpu_bsu_etc.at[0, "calculated_stop_loss_with_tp_3_to_1_achieved"] = stop_loss_achieved
    ...
    df_with_level_atr_bpu_bsu_etc.at[0, "human_datetime_when_calculated_sl_with_tp_3_to_1_was_achieved"] = human_date_and_time_when_sl_was_achieved
    ...
    df_with_level_atr_bpu_bsu_etc.at[0, "calculated_stop_loss_with_tp_4_to_1_achieved"] = stop_loss_achieved
    ...
    df_with_level_atr_bpu_bsu_etc.at[0, "human_datetime_when_calculated_sl_with_tp_4_to_1_was_achieved"] = human_date_and_time_when_sl_was_achieved
    ...
    take_profit_achieved, \
        ...
        return_bool_if_sl_or_tp_were_achieved_in_breakout_of_ath(table_with_ohlcv_data_df_slice_numpy_array,
                                                                 ...
                                                                 technical_stop_loss,
                                                                 take_profit_when_stop_loss_is_technical_3_to_1)
    df_with_level_atr_bpu_bsu_etc.at[0, "technical_take_profit_3_to_1_achieved"] = take_profit_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "stop_loss_with_technical_tp_3_to_1_achieved"] = stop_loss_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "neither_technical_tp_3_to_1_or_sl_3_to_1_achieved"] = neither_tp_or_sl_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "human_datetime_when_technical_tp_3_to_1_was_achieved"] = human_date_and_time_when_tp_was_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "human_datetime_when_sl_with_technical_tp_3_to_1_was_achieved"] = human_date_and_time_when_sl_was_achieved
    ##################################
    take_profit_achieved, \
        stop_loss_achieved, \
        neither_tp_or_sl_achieved, \
        ...
        return_bool_if_sl_or_tp_were_achieved_in_breakout_of_ath(table_with_ohlcv_data_df_slice_numpy_array,
                                                                 ...
                                                                 technical_stop_loss,
                                                                 take_profit_when_stop_loss_is_technical_4_to_1)
    df_with_level_atr_bpu_bsu_etc.at[0, "technical_take_profit_4_to_1_achieved"] = take_profit_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "stop_loss_with_technical_tp_4_to_1_achieved"] = stop_loss_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "neither_technical_tp_4_to_1_or_sl_4_to_1_achieved"] = neither_tp_or_sl_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "human_datetime_when_technical_tp_4_to_1_was_achieved"] = human_date_and_time_when_tp_was_achieved
    df_with_level_atr_bpu_bsu_etc.at[0, "human_datetime_when_sl_with_technical_tp_4_to_1_was_achieved"] = human_date_and_time_when_sl_was_achieved
                                ...
                                if number_of_last_row_in_np_array_row_slice>row_of_last_ath_which_is_limit_level_also+3:
                                    ...
                                    if number_of_bars_which_fulfil_suppression_to_ath >= 3:
                                        ...
                                        try:
                                            ...
                                            open_of_bar_next_day_after_next_day_after_breakout_bar = np.nan
                                            closeof_bar_next_day_after_next_day_after_breakout_bar = np.nan
                                            timestamp_of_bar_next_day_after_next_day_after_breakout_bar=np.nan
                                            ...
                                            try:
                                                close_of_bar_next_day_after_breakout_bar=\
                                                    table_with_ohlcv_data_df_slice_numpy_array[number_of_last_row_in_np_array_row_slice+1][4]
                                            except:
                                                pass
                                            ...
                                            try:
                                                ...
                                                timestamp_of_bar_next_day_after_next_day_after_breakout_bar=\
                                                    table_with_ohlcv_data_df_slice_numpy_array[number_of_last_row_in_np_array_row_slice+2][0]
                                            except:
                                                pass
                                            ...
                                            #check if breakout bar exists
                                            if open_of_breakout_bar and close_of_breakout_bar:
                                                # print(f"open_of_breakout_bar={open_of_breakout_bar}")
                                                # print(f"close_of_breakout_bar={close_of_breakout_bar}")
                                                # if open_of_breakout_bar>=close_of_bar_before_breakout:
                                                if volume_of_breakout_bar>=750:

                                                    distance_between_current_ath_and_breakout_bar_open = \
                                                        current_ath_in_iteration_over_numpy_array-open_of_breakout_bar
                                                    distance_between_current_ath_and_breakout_bar_close = \
                                                        close_of_breakout_bar-current_ath_in_iteration_over_numpy_array
                                                    if distance_between_current_ath_and_breakout_bar_open>0:
                                                        if (distance_between_current_ath_and_breakout_bar_open > advanced_atr * 0.05) and\
                                                            (distance_between_current_ath_and_breakout_bar_close > advanced_atr * 0.05):

                                                            #check if bar next day after break out bar exists
                                                            if open_of_bar_next_day_after_breakout_bar:
                                                                # check if bar after breakout bar opens and closes within acceptable range
                                                                if (close_of_breakout_bar<=open_of_bar_next_day_after_breakout_bar) and \
                                                                        ((open_of_bar_next_day_after_breakout_bar-current_ath_in_iteration_over_numpy_array)<2*advanced_atr):
                                                                    if (close_of_breakout_bar <= close_of_bar_next_day_after_breakout_bar) and \
                                                                            ((close_of_bar_next_day_after_breakout_bar - current_ath_in_iteration_over_numpy_array) < 2 * advanced_atr):
                                                                        #check if bar after bar after breakout bar exists (day two after breakout)
                                                                        if open_of_bar_next_day_after_next_day_after_breakout_bar:
                                                                            if open_of_bar_next_day_after_next_day_after_breakout_bar>=close_of_breakout_bar:
                                                                                ...
                                                                                print (
                                                                                    ...
                                                                                    f" close_of_bar_before_breakout={close_of_bar_before_breakout} open_of_breakout_bar={open_of_breakout_bar}" )
                                                                                print(f"open_of_breakout_bar={open_of_breakout_bar}")
                                                                                print (
                                                                                    f"distance_between_current_ath_and_breakout_bar_open={distance_between_current_ath_and_breakout_bar_open}" )
                                                                                list_of_stocks_approaching_ath.append ( stock_name )
                                                                                ...
                                                                                df_with_level_atr_bpu_bsu_etc.loc[
                                                                                    0 , "open_of_bar_next_day_after_breakout_bar"] = open_of_bar_next_day_after_breakout_bar
                                                                                df_with_level_atr_bpu_bsu_etc.loc[
                                                                                    0 , "close_of_bar_next_day_after_breakout_bar"] = close_of_bar_next_day_after_breakout_bar
                                                                                df_with_level_atr_bpu_bsu_etc.loc[
                                                                                    0 , "open_of_day_two_bar"] = open_of_bar_next_day_after_next_day_after_breakout_bar
                                                                                df_with_level_atr_bpu_bsu_etc.loc[
                                                                                    0, "timestamp_of_day_two_bar"] = timestamp_of_bar_next_day_after_next_day_after_breakout_bar
                                                                                df_with_level_atr_bpu_bsu_etc.loc[
                                                                                    0, "position_entry_timestamp"] = timestamp_of_bar_next_day_after_next_day_after_breakout_bar
                                                                            ...
                                                                            #in this case bar on day two after breakout does not exist
                                                                            date_and_time_of_approaching_to_ath , date_of_approaching_to_ath = get_date_with_and_without_time_from_timestamp (
                                                                                first_several_rows_in_np_array_slice[
                                                                                    -1][0] )
                                                                            print (
                                                                            ...
                                                                            print (
                                                                                f"distance_between_current_ath_and_breakout_bar_open={distance_between_current_ath_and_breakout_bar_open}" )
                                                                            list_of_stocks_approaching_ath.append (
                                                                                stock_name )
                                                                            date_and_time_of_ath , date_of_ath = get_date_with_and_without_time_from_timestamp (
                                                                                timestamp_of_current_ath )
                                                                            date_and_time_of_current_timestamp , date_of_current_timestamp = get_date_with_and_without_time_from_timestamp (
                                                                ...
                                                                print ( f"open_of_breakout_bar={open_of_breakout_bar}" )
                                                                print ( f"close_of_breakout_bar={close_of_breakout_bar}" )
                                                                print ( f"open_of_bar_next_day_after_breakout_bar_when_it_does_not_exist_yet={open_of_bar_next_day_after_breakout_bar}" )
                                                                print (
                                                                    f"distance_between_current_ath_and_breakout_bar_open={distance_between_current_ath_and_breakout_bar_open}" )
                                                                list_of_stocks_approaching_ath.append ( stock_name )
                                                                date_and_time_of_ath , date_of_ath = get_date_with_and_without_time_from_timestamp (
                                                ...
                                                # Only information about pre-breakout bar available
                                                print ( f"open_of_breakout_bar={open_of_breakout_bar}" )
                                                date_and_time_of_approaching_to_ath , date_of_approaching_to_ath = get_date_with_and_without_time_from_timestamp (
                                                    first_several_rows_in_np_array_slice[-1][0] )
                                                print (
                                                ...
                                                print (
                                                    f"for stock {stock_name} on {date_of_approaching_to_ath} approached to ath={current_ath_in_iteration_over_numpy_array}"
                                                    f" close_of_bar_before_breakout={close_of_bar_before_breakout} (breakout_bar NON-existent) open_of_breakout_bar={open_of_breakout_bar}"
                                                ...
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "exchange"] = exchange
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "short_name"] = short_name
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "ath"] = current_ath_in_iteration_over_numpy_array
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "advanced_atr"] = advanced_atr

                                                df_with_level_atr_bpu_bsu_etc.at[0 , "advanced_atr_over_this_period"] = \
                                                    advanced_atr_over_this_period
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "high_of_bsu"] = current_ath_in_iteration_over_numpy_array
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "volume_of_bsu"] = volume_in_current_ath
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "timestamp_of_bsu"] = timestamp_of_current_ath
                                                df_with_level_atr_bpu_bsu_etc.at[0 , "human_date_of_bsu"] = date_of_ath
                                                ...
                                                df_with_level_atr_bpu_bsu_etc.at[0, "open_of_bar_next_day_after_breakout_bar"] = open_of_bar_next_day_after_breakout_bar
                                                df_with_level_atr_bpu_bsu_etc.at[0, "close_of_bar_next_day_after_breakout_bar"] = close_of_bar_next_day_after_breakout_bar
...
##################################################################################
                ...
                #                                         open_of_breakout_bar - current_atl_in_iteration_over_numpy_array
                #                                     distance_between_current_atl_and_breakout_bar_close = \
                #                                         current_atl_in_iteration_over_numpy_array-close_of_breakout_bar
                #                                     if distance_between_current_atl_and_breakout_bar_open>0:
                #                                         #breakout bar opens and closes further than 5%ATR from the level
                #                                         if (distance_between_current_atl_and_breakout_bar_open > advanced_atr * 0.05) and\
                #                                             (distance_between_current_atl_and_breakout_bar_close > advanced_atr * 0.05):
                #                                             # check if bar next day after break out bar exists
                #                                             if open_of_bar_next_day_after_breakout_bar:
                #                                                 #check if bar after breakout bar opens and closes within acceptable range
                #                                                 if (close_of_breakout_bar >= open_of_bar_next_day_after_breakout_bar) and \
                #                                                         ((current_atl_in_iteration_over_numpy_array-open_of_bar_next_day_after_breakout_bar) < 2 * advanced_atr):
                #                                                     if (close_of_breakout_bar >= close_of_bar_next_day_after_breakout_bar) and \
                #                                                             ((current_atl_in_iteration_over_numpy_array - close_of_bar_next_day_after_breakout_bar) < 2 * advanced_atr):
                #                                                         #check if bar on day two after breakout exists
                #                                                         if open_of_bar_next_day_after_next_day_after_breakout_bar:
                #                                                             if open_of_bar_next_day_after_next_day_after_breakout_bar<=close_of_breakout_bar:
                #
                #
                ...
                #                                                                     if_exists = 'append' )
                #                                                         else:
                #                                                             #in this case day number two after breakout doesnot exist
                #                                                             date_and_time_of_approaching_to_atl , date_of_approaching_to_atl = \
                #                                                                 get_date_with_and_without_time_from_timestamp (
                #                                                                     first_several_rows_in_np_array_slice[
                #                                                                         -1][0] )
                ...
                #                                                             date_and_time_of_atl , date_of_atl = get_date_with_and_without_time_from_timestamp (
                #                                                                 timestamp_of_current_atl )
                #                                                             list_of_stocks_approaching_atl.append (
                #                                                                 stock_name )
                #                                                             df_with_level_atr_bpu_bsu_etc = pd.DataFrame ()
                #                                                             df_with_level_atr_bpu_bsu_etc.loc[
                #                                                                 0 , "ticker"] = stock_name
                ...
                #                                                     f"close_of_breakout_bar={close_of_breakout_bar}" )
                #                                                 print (
                #                                                     f"open_of_bar_next_day_after_breakout_bar_when_it_does_not_exist_yet={open_of_bar_next_day_after_breakout_bar}" )
                #                                                 print (
                #                                                     f"distance_between_current_atl_and_breakout_bar_open={distance_between_current_atl_and_breakout_bar_open}" )
                #                                                 list_of_stocks_approaching_atl.append ( stock_name )
                #                                                 date_and_time_of_atl , date_of_atl = get_date_with_and_without_time_from_timestamp (
                #                                                     timestamp_of_current_atl )
                #                                                 date_and_time_of_current_timestamp , date_of_current_timestamp = get_date_with_and_without_time_from_timestamp (
                ...
                #                                                     if_exists = 'append' )
                #
                #                             #breakout bar does not exist yet. Only pre breakout bar info available
                #                             else:
                #                                 date_and_time_of_approaching_to_atl , date_of_approaching_to_atl = \
                #                                     get_date_with_and_without_time_from_timestamp (
                #                                         first_several_rows_in_np_array_slice[-1][0] )
                ...
                #                                     f"number_of_bars_which_fulfil_suppression_to_atl={number_of_bars_which_fulfil_suppression_to_atl}" )
                #                                 print (
                #                                     f"for stock {stock_name} on {date_of_approaching_to_atl} prebreakout bar close approached "
                #                                     f"to atl={current_atl_in_iteration_over_numpy_array}"
                #                                     f" close_of_bar_before_breakout={close_of_bar_before_breakout} (breakout bar non existent) open_of_breakout_bar={open_of_breakout_bar}" )
                #
                #                                 date_and_time_of_atl , date_of_atl = get_date_with_and_without_time_from_timestamp (
                #                                     timestamp_of_current_atl )
                #                                 list_of_stocks_approaching_atl.append ( stock_name )
...
if __name__=="__main__":
    ...
    table_where_ticker_which_may_have_fast_breakout_situations_from_ath_will_be =\
        "breakout_situations_of_ath_position_entry_on_day_after_next_day"
    table_where_ticker_which_may_have_fast_breakout_situations_from_atl_will_be =\
        "breakout_situations_of_atl_position_entry_on_day_after_next_day"
