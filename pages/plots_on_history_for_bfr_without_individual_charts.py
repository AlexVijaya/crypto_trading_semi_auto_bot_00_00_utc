...
def plot_deposit_by_end_of_period_for_calc_and_tech_sl_and_plot_number_of_trades_within_n_days(
        table_name,
        engine_for_db_levels_formed_by_highs_and_lows_for_cryptos_0000_hist,
        dict_of_identical_queries_for_each_table,
        number_of_prev_and_next_days,
    ...
    if "max_profit_target_multiple_when_sl_technical" in df_with_resulting_table_of_certain_models.columns or\
        ...
        for number_of_y_axis_minus_two, table_name in enumerate(dict_of_identical_queries_for_each_table):
            query=dict_of_identical_queries_for_each_table[table_name]
            ...
            fig.add_trace(go.Scatter(x=df_with_resulting_table_of_certain_models1["human_datetime_of_order_placement_bar"],
                                     ...
                                     name=f"for {table_name.replace('''current_''','''''')} <br>number of order placement within next and prev {number_of_prev_and_next_days} days "  # Assign a name to the trace
        ...
        for number_of_y_axis_minus_two, table_name in enumerate(dict_of_identical_queries_for_each_table):
            ...
            if "max_profit_target_multiple_when_sl_technical" in df_with_resulting_table_of_certain_models1.columns:
                ...
                fig.add_trace(go.Scatter(x=df_with_resulting_table_of_certain_models1["human_datetime_of_order_placement_bar"],
                                         ...
                                         name=f"max_profit_target_multiple_when_sl_technical {count_of_non_zero}/{count_of_zero} for {table_name.replace('''current_''','''''')}"  # Assign a name to the trace
            ...
            if "max_profit_target_multiple_when_sl_calculated" in df_with_resulting_table_of_certain_models1.columns:
                ...
                fig.add_trace(
                    go.Scatter(x=df_with_resulting_table_of_certain_models1["human_datetime_of_order_placement_bar"],
                               ...
                               mode='markers', yaxis=f'y{number_of_y_axis_minus_two + 2}', visible='legendonly',
                               name=f"max_profit_target_multiple_when_sl_calculated {count_of_non_zero}/{count_of_zero} for {table_name.replace('''current_''', '''''')}"
        ...
        fig.update_layout(height=height_of_line_chart * 7, width=width_of_line_chart,
                          title="Deposit by end of given period")
            ...
            if model=="New ATL within last two days":
                ...
                try:
                    df_with_resulting_table_of_certain_models = \
                        return_df_from_postgres_sql_table(query, table_name,
                                                          engine_for_db_levels_formed_by_highs_and_lows_for_cryptos_0000_hist)
                    plot_deposit_by_end_of_period_for_calc_and_tech_sl_and_plot_number_of_trades_within_n_days(table_name, engine_for_db_levels_formed_by_highs_and_lows_for_cryptos_0000_hist,
                                                  dict_of_identical_queries_for_each_table,
                                                  number_of_prev_and_next_days,
                                                  engine_for_ohlcv_data_for_stocks_0000,
                                                  df_with_resulting_table_of_certain_models,
