...
def return_tuple_of_trading_pairs_with_exchange(tuple_of_trading_pairs_with_exchange):
    plots_per_page = 10
    # Define the total number of plots
    total_plots = len(tuple_of_trading_pairs_with_exchange)
    # Calculate the number of pages
...
def return_df_from_postgres_sql_table1(table_name,_engine_name):
    ...
    try:
        query=f'''SELECT t.*
            ...
            LEFT JOIN (
                SELECT base, timestamp_of_bsu
                FROM public."{table_name}"
                GROUP BY base, timestamp_of_bsu
                HAVING COUNT(DISTINCT exchange) = 1
            ) sub
            ON t.base = sub.base AND t.timestamp_of_bsu = sub.timestamp_of_bsu
            WHERE sub.base IS NULL and min_volume_in_usd_over_last_n_days>=100000
            and buy_or_sell_order_was_touched=True 
            and max_distance_from_level_price_in_this_bar_in_atr<4;''' # and exchange IN ({exchange_conditions})
        ...
        query = f'''SELECT *
                    ...
                    AND min_volume_in_usd_over_last_n_days >= 100000
                    AND buy_or_sell_order_was_touched = TRUE
                    AND max_distance_from_level_price_in_this_bar_in_atr < 1;'''
        df_with_resulting_table_of_certain_models = \
            pd.read_sql_query(query,
                              _engine_name)
    except:
...
def streamlit_func():
    ...
    with st.form("bfr_criteria_selection_form"):
        ...
        max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number=np.nan
        max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number=np.nan
        show_only_stopped_out_positions_when_sl_is_technical=False
        show_only_stopped_out_positions_when_sl_is_calculated=False
        sql_query_part_if_max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number = ""
        sql_query_part_if_max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number = ""
        if table_name=="current_breakout_situations_of_ath_position_entry_next_day":
            max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number = st.number_input(
                label="max_profit_target_multiple_when_sl_technical is >= than this number", value=0)
            sql_query_part_if_max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number = \
                f" and max_profit_target_multiple_when_sl_technical>={max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number}"
            max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number = st.number_input(
                label="max_profit_target_multiple_when_sl_calculated is >= than this number", value=0)
            sql_query_part_if_max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number = \
                f" and max_profit_target_multiple_when_sl_calculated>={max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number}"
            show_only_stopped_out_positions_when_sl_is_technical = st.checkbox(
                "show_only_stopped_out_positions_when_sl_is_technical")
            show_only_stopped_out_positions_when_sl_is_calculated = st.checkbox(
                "show_only_stopped_out_positions_when_sl_is_calculated")
        if table_name=="current_breakout_situations_of_atl_position_entry_next_day":
            max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number = st.number_input(
                label="max_profit_target_multiple_when_sl_technical is >= than this number", value=0)
            sql_query_part_if_max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number = \
        ...
        sql_query_part_if_to_show_only_stopped_out_positions_when_sl_is_technical=""
        sql_query_part_if_to_show_only_stopped_out_positions_when_sl_is_calculated=""
        if show_only_stopped_out_positions_when_sl_is_calculated==True:
            sql_query_part_if_to_show_only_stopped_out_positions_when_sl_is_calculated=f" and max_profit_target_multiple_when_sl_calculated=0"
            sql_query_part_if_max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number=""
        if show_only_stopped_out_positions_when_sl_is_technical == True:
            sql_query_part_if_to_show_only_stopped_out_positions_when_sl_is_technical = f" and max_profit_target_multiple_when_sl_technical=0"
            sql_query_part_if_max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number=""
        ...
        number_of_prev_and_next_days=st.number_input(label="number_of_prev_and_next_days", value=15,min_value=1, step=1)
        ...
        form_submit_button_is_pressed = st.form_submit_button('Submit my picks now')
        tuple_of_trading_pairs_with_exchange=tuple()
        ...
        if form_submit_button_is_pressed:
            ...
            query=f'''SELECT *
                            ...
                            AND buy_or_sell_order_was_touched = {buy_or_sell_order_was_touched} {sql_query_part_if_pair_is_traded_with_margin_or_swap_is_available} {sql_query_part_if_to_show_only_stopped_out_positions_when_sl_is_calculated} 
                            {sql_query_part_if_to_show_only_stopped_out_positions_when_sl_is_technical} {sql_query_part_if_max_profit_target_multiple_when_sl_calculated_is_more_or_equal_than_this_number} 
                            {sql_query_part_if_max_profit_target_multiple_when_sl_technical_is_more_or_equal_than_this_number} AND
            ...
            if model=="Breakout of ATL with position entry next day after breakout":
                ...
                #         # index_of_trading_pair_to_select = tuple_of_trading_pairs_with_exchange.index(trading_pair_to_select)
                #
                #         with st.expander(f"{trading_pair_to_select}"):
    ...
    #
    #     with st.expander(f"{trading_pair_to_select}"):
    #         plot_multiple_charts_on_one_page(
    #             connection_to_db_levels_formed_by_highs_and_lows_for_cryptos_0000_hist, table_name,
    #             index_of_trading_pair_to_select,
    #             trading_pair_to_select, df_with_resulting_table_of_certain_models,
    #             engine_for_ohlcv_data_for_stocks_0000,
    #             engine_for_ohlcv_low_volume_data_for_stocks_0000, height, width)

    # Define the function that plots the charts for a batch of trading pairs
    # def plot_batch_of_charts(trading_pairs):
    #     for index, trading_pair in enumerate(trading_pairs):
    #         # Call your plot function for each trading pair
    #         plot_multiple_charts_on_one_page(
    #             connection_to_db_levels_formed_by_highs_and_lows_for_cryptos_0000_hist,
    #             table_name,
    #             index,
    ...
    #         )
    #
    # # Placeholder for the current batch index
    # current_batch_index = st.session_state.current_batch_index if 'current_batch_index' in st.session_state else 0
    #
    # # Specify the batch size
    # batch_size = 10
    #
    # # Button to plot the next batch of trading pairs
    # if st.button("Plot next 10 trading pairs"):
    #     # Extract the next batch of trading pairs
    #     batch_trading_pairs = tuple_of_trading_pairs_with_exchange[
    #                           current_batch_index:current_batch_index + batch_size]
    #     # Plot the batch of charts
    #     plot_batch_of_charts(batch_trading_pairs)
    #     # Update the current batch index
    #     current_batch_index += batch_size
    #     st.session_state.current_batch_index = current_batch_index
    # tuple_of_trading_pairs_with_exchange = \
    #     add_sidebar_and_return_tuple_of_trading_pairs_with_exchange(tuple_of_trading_pairs_with_exchange)
    # ##############################3
    ...
    for index_of_trading_pair_to_select, trading_pair_to_select in enumerate(tuple_of_trading_pairs_with_exchange):
        ...
        string_index_in_tuple_double_underscore_trading_pair_to_select_list.append(string_index_in_tuple_double_underscore_trading_pair_to_select)

    # df_trading_pair_to_select = pd.DataFrame(string_index_in_tuple_double_underscore_trading_pair_to_select_list, columns=["trading_pair_to_select"])
    df_with_resulting_table_of_certain_models.to_sql(
        table_name,
        engine_for_db_intermediate_db_for_storing_selected_pairs_from_form,
        if_exists='replace')

    st.link_button("See the selected charts","http://localhost:8502/plots_on_history_for_bfr_with_form_on_another_page")


    # radio_button_for_one_trading_pair=st.radio(options=string_index_in_tuple_double_underscore_trading_pair_to_select_list,
